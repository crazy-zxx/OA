...
var FCKPanel = function( parentWindow )
{
	...
	if ( FCKBrowserInfo.IsIE )
	{
		...
		// The popup has to be created before playing with entity hacks, see #1666.
		this._Popup	= this._Window.createPopup() ;

		// this._Window cannot be accessed while playing with entity hacks, but local variable is ok.
		// See #1666.
		var pDoc = this._Window.document ;

		// This is a trick to IE6 (not IE7). The original entity must be set
		// before creating the popup, so we are able to take a refence to the
		// document inside of it, and the set the proper entity for it. (#123)
		...
		// Set the proper entity inside the popup.
