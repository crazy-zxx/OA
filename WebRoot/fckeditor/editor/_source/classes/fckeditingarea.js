...
FCKEditingArea.prototype.Start = function( html, secondCall )
{
	...
	{
		// For FF, document.entity must be set only when different, otherwhise
		// we'll strangely have "Permission denied" issues.
		if ( FCK_IS_CUSTOM_DOMAIN )
			html = '<script>document.entity="' + FCK_RUNTIME_DOMAIN + '";</script>' + html ;
		...
		if ( FCK_IS_CUSTOM_DOMAIN && FCKBrowserInfo.IsIE )
		{
			...
			oIFrame.src = 'javascript:void( (function(){' +
				'document.open() ;' +
				'document.entity="' + document.domain + '" ;' +
